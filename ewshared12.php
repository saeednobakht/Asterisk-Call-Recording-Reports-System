...
class cLanguage {
	...
	function __construct($langfolder = "", $langid = "") {
		global $gsLanguage, $gsDirection;
		global $IsRTL, $RTLFile, $DIRFile;
		if ($langfolder <> "")
		...
		$this->Dir = $this->LoadFileDir($this->GetFileName($gsLanguage));
		$this->IsRTL = (strtoupper($this->Dir) == "RTL");
		$this->Align = ($this->IsRTL) ? "right" : "left";
		$this->RTLFile = ($this->IsRTL) ? "-rtl" : "";
		$this->DIRFile = ("-".$this->Dir);
		$gsDirection = (EW_CSS_FLIP) ? $this->Dir : "";
		$IsRTL = (EW_CSS_FLIP) ? $this->IsRTL : FALSE;
		$RTLFile = (EW_CSS_FLIP) ? $this->RTLFile : "";
		$DIRFile = (EW_CSS_FLIP) ? $this->DIRFile : "";
	...
	// Load language file direction
	function LoadFileDir($File) {
		if (EW_USE_DOM_XML) {
			$this->Phrases = new cXMLDocument();
			if ($this->Phrases->Load($File))
				return $this->GetNodeAtt($this->Phrases->DocumentElement(), "dir");
		} else {
			$ar = ew_Xml2Array(substr(file_get_contents($File), 0, 512)); // Just read the first part
			return (is_array($ar)) ? @$ar['ew-language']['attr']['dir'] : "";
		}
	}
	...
	#	if ($this->LocalePhrase("time_zone") <> "") $DEFAULT_TIME_ZONE = $this->LocalePhrase("time_zone"); //ntx
		...
		foreach ($this->ItemData as $item) {
			if ($this->RenderItem($item)) {
				...
				if ($item->GroupTitle && (!$this->IsRoot || !$this->MenuRootGroupTitleAsSubMenu)) { // Group title
					...
					if (!is_null($item->SubMenu)) {
						foreach ($item->SubMenu->ItemData as $subitem) {
							...
							$aclass = "";
							$MenuName = @strstr($item->Url, 'list.php', true);
						#	if (!$subitem->IsCustomUrl && ew_CurrentPage() == ew_GetPageName($subitem->Url) || $subitem->IsCustomUrl && $cururl == $subitem->Url) {
							if (!$subitem->IsCustomUrl && $MenuName == $Page->TableName || $subitem->IsCustomUrl && $cururl == $subitem->Url) {
				...
				#	if (!$item->IsCustomUrl && ew_CurrentPage() == ew_GetPageName($item->Url) || $item->IsCustomUrl && $cururl == $item->Url) {
					if (!$item->IsCustomUrl && $MenuName == $Page->TableName || $item->IsCustomUrl && $cururl == $item->Url) {
						if ($this->IsRoot)
							ew_AppendClass($liclass, $this->MenuActiveItemClassName);
						else
...
// Check if HTTPS
function ew_IsHttps() {
	$HTTPS = (ew_ServerVar("HTTPS") <> "" && ew_ServerVar("HTTPS") <> "off");
	if (!$HTTPS) {
