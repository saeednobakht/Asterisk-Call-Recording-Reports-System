...
function CurrentProjectID() {
	...
	return "{274CC91E-1C95-40BB-9BB8-39D2A070EA8E}";
}
...
function ew_GetImgViewTag(&$fld, $val) {
	if (!ew_EmptyStr($val)) {
		...
		if (ew_IsImageFile($val)) {
			...
			$url = ew_GetImageUrl($fld, $wrkfile);
			$name = $val;
			if ($url <> "")
				$image = "<a" . $fld->LinkAttributes() . ">" . $name . "</a>";
		}
...
function ew_GetImageUrl($fld, $val, $resize = FALSE, $encrypt = EW_ENCRYPT_FILE_PATH, $urlencode = TRUE) {
	if (!ew_EmptyStr($val)) {
		if ($encrypt) {
			$key = EW_RANDOM_KEY . session_id();
			$fn = "ewfile12.php?t=" . ew_Encrypt($fld->TblName, $key) ."&fn=" . ew_Encrypt($val, $key);
			if ($resize)
				$fn .= "&width=" . $fld->ImageWidth . "&height=" . $fld->ImageHeight;
		} elseif ($resize) {
			$fn = "ewfile12.php?t=" . rawurlencode($fld->TblName) . "&fn=" . ew_UrlEncodeFilePath($val) .
				"&width=" . $fld->ImageWidth . "&height=" . $fld->ImageHeight;
		} else {
			$fn = $val;
			if ($urlencode)
...
class cField {
	...
	function __construct($tblvar, $tblname, $fldvar, $fldname, $fldexp, $fldbsexp, $fldtype, $flddtfmt, $upload, $fldvirtualexp, $fldvirtual, $forceselect, $fldvirtualsrch, $fldviewtag="", $fldhtmltag="") {
		...
		$this->DisplayValueSeparator = $Language->Phrase("Separator");
	}
	var $PlaceHolder = "";
	...
	function getPlaceHolder() {
		if ($this->FldTitle()) {
			return ew_HtmlEncode($this->FldTitle());
		} else {
	...
	function ListViewValue() {
		if ($this->FldDataType == EW_DATATYPE_XML) {
			...
			if ($value <> "") {
				$value2 = trim(preg_replace('/<[^(img|object)][^>]*>/i', '', strval($value)));
...
class cUpload {
	...
	function UploadFile() {
		...
		if (!$this->KeepFile && $this->FileName <> "" && !$this->UploadMultiple) {
			...
			if (file_exists($f)) {
				...
				$this->ImgWidthSrc = @$sizes[0];
				$this->ImgHeightSrc = @$sizes[1];
			}
		}
	...
	function SaveToFile($Path, $NewFileName, $OverWrite, $idx = -1) {
		if (!ew_Empty($this->Value)) {
			...
			if (!$OverWrite)
				$NewFileName = ew_UploadFileNameEx($Path, $NewFileName, 'savefile');
...
function ew_Header($cache, $charset = EW_CHARSET) {
	...
	if ($cache || ew_IsHttps() && $export <> "" && $export <> "print") { // Allow cache
		header("Cache-Control: max-age=31536000, public, must-revalidate");
		header('Pragma: public');
		header('Expires: '.date(DATE_RFC1123,  time() + 31536000));
	} else { // No cache
...
//	if(md5(substr($_SERVER['SERVER_NAME'], -9)) != 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx')exit();
//	if(($_SERVER['SERVER_ADDR'] != '127.0.0.1') && (!preg_match('/localhost/', $_SERVER['SERVER_NAME'])))exit();
//	eval(base64_decode("aWYobWQ1KHN1YnN0cigkX1NFUlZFUlsnU0VSVkVSX05BTUUnXSwgLTkpKSAhPSAneHh4eHh4eHh4eHh4eHh4eHh4eHh4eHh4eHh4eHh4eHgnKWV4aXQoKTs="));
...
#	$sep = ($fld) ? $fld->DisplayValueSeparator : ", "; //ntx
	$sep = ($fld) ? $fld->DisplayValueSeparator : $GLOBALS['Language']->Phrase("Separator");
	return (is_array($sep)) ? @$sep[$idx - 1] : $sep;
}
...
#	return ", "; //ntx
	return $GLOBALS['Language']->Phrase("Separator");
}
...
include_once($EW_RELATIVE_PATH . "phpmailer/class.phpmailer.php");
include_once($EW_RELATIVE_PATH . "phpmailer/class.smtp.php");
...
function ew_SendEmail($sFrEmail, $sToEmail, $sCcEmail, $sBccEmail, $sSubject, $sMail, $sFormat, $sCharset, $sSmtpSecure = "", $arAttachments = array(), $arImages = array(), $arProperties = NULL) {
	...
	// ntx+
	if (EW_RECIPIENT_EMAIL) {
		$mail->FromName = EW_RECIPIENT_EMAIL;

	//	$mail->Subject  = $mail->Subject .' - '. EW_RECIPIENT_EMAIL;
		$mail->Subject  = $mail->Subject .' - '. date('Y-m-d H:i:s');
	}
...
//   $rsnew['image'] = ew_UploadFileNameEx(ew_UploadPathEx(True, $this->image->UploadPath), $this->image->Upload->FileName, 'identify', $this->id->CurrentValue);
// }
// Extended define auto filename
// original = 1> filename.bin -> filename.bin 2> filename.bin -> filename.bin - overwrite if file exist.
// unique   = 1> filename.bin -> filename.bin 2> filename.bin -> filename(1).bin - auto add number.
// identify = 1> definename.bin -> definename.bin 2> definename.bin -> definename.bin - get file name from $sFileId.
// sequence = 1> filename.bin -> yyyymmddhhmmss001.bin 2> filename.bin -> yyyymmddhhmmss002.bin - get sequence number from $sFileId.
// default  = 1> filename.bin -> yyyymmddhhmmss.bin 2> filename.bin -> yyyymmddhhmmss(1).bin - get file name from full date time.
function ew_UploadFileNameEx($folder, $sFileName, $sFileMode = "default", $sFileId = 0) {

	// By default, ew_UniqueFileName() is used to get an unique file name
	// You can change the logic here
//	$sFileName  =  ew_DefaultFileName(strtolower(strrchr($sFileName, ".")));
//	$sOutFileName = ew_UniqueFilename($folder, $sFileName);

	$xPathInfo  = pathinfo($sFileName);
	$sExtension = $xPathInfo['extension'];
	if ($sFileMode != "savefile")
	{
		if ($sFileMode == "original")
		{
			$sOutFileName = ew_UniqueFilename($folder, $sFileName, FALSE, "original");
		} 
		elseif ($sFileMode == "unique")
		{
			$sOutFileName = ew_UniqueFilename($folder, $sFileName, TRUE, "unique");
		} 
		elseif ($sFileMode == "identify")
		{
			$sFileName    = $sFileId . $sExtension;
			$sOutFileName = ew_UniqueFilename($folder, $sFileName, FALSE, "identify");
		} 
		elseif ($sFileMode == "sequence")
		{
			$sFileName    = ew_DefaultFileName($sExtension, FALSE, $sFileId, 4, 0, STR_PAD_LEFT);
			$sOutFileName = ew_UniqueFilename($folder, $sFileName, FALSE, "sequence");
		} 
		elseif (!$sFileMode || $sFileMode == "default")
		{
			$sFileName    = ew_DefaultFileName($sExtension, TRUE);
			$sOutFileName = ew_UniqueFilename($folder, $sFileName, TRUE);
		}
	} else
	{
...
function ew_UniqueFilename($folder, $orifn, $indexed = FALSE, $sFileMode = NULL) {

//	if ($orifn == "")
//		$orifn = date("YmdHis") . ".bin";

	if ($orifn == "") {
		$orifn = ew_DefaultFileName(".bin");
	}
	$info = pathinfo(preg_replace('/\s/', '_', $orifn));

//	$newfn = mb_strtolower($info["basename"]);
	if ($sFileMode == "original") {
		$newfn = $info["basename"];
		$newfn = preg_replace('/\(\d+\)/i', '', $newfn);
	} else {
...
//	$newfn = $info["basename"];
	...
	if ($sFileMode == "original" || $sFileMode == "sequence") {
		if (@file_exists($destpath)) {
			@unlink($destpath);
		}
	}
	$i = 1;
	if ($indexed && preg_match('/\(\d+\)/i', $newfn, $matches)) // Match '(n)' at the end of the file name
		$i = intval($matches[1]);
	if (!file_exists($folder) && !ew_CreateFolder($folder))
		die("Folder does not exist: " . $folder);
	if (!$sFileMode || $sFileMode == 'unique' || $indexed) {
		while (file_exists(ew_Convert(EW_ENCODING, EW_FILE_SYSTEM_ENCODING, $destpath))) {
			...
			$file_name = preg_replace('/\(\d+\)/i', '', $info["filename"]); // Remove "(n)" at the end of the file name
...
// function to create a default file name(yyyymmddhhmmss.bin) or (yyyymmddhhmmss001.bin)
function ew_DefaultFileName($extension = NULL, $millisecond = TRUE, $input = 0, $length = 0, $string = 0, $type = STR_PAD_LEFT) {

//	return date("YmdHi") . (ceil(date('s')/10).'0') . $extension;
	$FileName = date("YmdHis");
	if ($millisecond) {
		$FileName.= substr(microtime(), 2, 6);
	}
	if ($input && $length) {
		if ($string !== false && $type)
			$FileName.= str_pad($input, $length, $string, $type);
		else
			$FileName.= str_pad($input, $length, 0, STR_PAD_LEFT);
	}
	if ($extension) {
...
function ew_RemoveXSS($val) {
	...
	while ($found == true) { 
	   ...
	   for ($i = 0; $i < sizeof($ra); $i++) { 
		  ...
		  if (!preg_match('/(plugins\/cssanim)/i', $val) || $ra[$i] != '<script') //ntx+
...
// Include extra code
include_once "extrafn.php";

// Include Hijri_Shamsi code
include_once "phpinc/jdf.inc.php";
?>
