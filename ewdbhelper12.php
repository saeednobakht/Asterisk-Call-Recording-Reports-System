...
class cfreepbx_db extends cDbHelper {
	...
	function &Connect($info = NULL) {
		...
		$this->Host =     $EW_CONN["DB"]["host"];
		$this->Port =     $EW_CONN["DB"]["port"];
		$this->Username = $EW_CONN["DB"]["user"];
		$this->Password = $EW_CONN["DB"]["pass"];
		$this->DbName =   $EW_CONN["DB"]["db"];
		if (!$info) {
